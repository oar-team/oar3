...
@click.command()
@click.option('--zeromq-bind-uri', help="ZeroMQ bind uri",
              default=get_default_bind)
@click.option('--api-host', default="http://localhost/oarapi-priv",
              help="OAR Rest API url")
...
def cli(zeromq_bind_uri, api_host, username,
        password, walltime, nodes,
        submit_count, submit_interval):
    ...
    app = DefaultScheduler(command=command, api_host=api_host,
                           zeromq_bind_uri=zeromq_bind_uri,
                           api_credentials=api_credentials, nodes=max_nodes,
                           walltime=walltime, nodes_range=nodes_range)
