...
def test_db_properties_1(monkeypatch):
    now = get_date()
    insert_job(
        res=[(60, [("resource_id=2", "")])], properties="network_address='localhost1'"
    )
    insert_job(
        res=[(60, [("resource_id=2", "")])], properties="network_address='localhost1'"
    )
...
def test_db_properties_2(monkeypatch):
    now = get_date()
    insert_job(
        res=[(60, [("resource_id=2", "network_address='localhost1'")])], properties=""
    )
    insert_job(
        res=[(60, [("resource_id=2", "network_address='localhost1'")])], properties=""
    )
...
def _test_db_placeholder_1(monkeypatch):
    now = get_date()
    insert_job(
        res=[(60, [("resource_id=4", "")])], properties="", types=["placeholder=yop"]
    )
