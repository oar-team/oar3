...
def test_oarsub_directory(monkeypatch):
    runner = CliRunner()
    result = runner.invoke(
        cli, ["-q default", "-d", "/home/robin/batcave", '"sleep 1"']
    )
    print(result.output)
    job = db["Job"].query.one()
...
def test_oarsub_stdout_stderr(monkeypatch):
    runner = CliRunner()
    result = runner.invoke(
        cli, ["-q default", "-O", "foo_%jobid%o", "-E", "foo_%jobid%e", '"sleep 1"']
    )
    print(result.output)
    job = db["Job"].query.one()
    assert result.exit_code == 0
...
def test_oarsub_admission_queue_1(monkeypatch):
    db["AdmissionRule"].create(rule=("if user == 'yop':" "    queue= 'default'"))
...
def test_oarsub_sleep_property_error(monkeypatch):
    runner = CliRunner()
    result = runner.invoke(
        cli, ["-q default", "-l resource_id=4", "-p yopyop SELECT", '"sleep 1"']
    )
    print(result.output)
