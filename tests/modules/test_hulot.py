...
def minimal_db_initialization(request):
    with db.session(ephemeral=True):
        ...
        for i in range(2):
            db["Resource"].create(
                network_address="localhost0", available_upto=2147483646
            )
        for i in range(2):
            db["Resource"].create(
                network_address="localhost1", available_upto=2147483646
            )
        for i in range(2):
            db["Resource"].create(
                network_address="localhost2", state="Absent", available_upto=2147483646
            )
        for i in range(2):
            db["Resource"].create(
                network_address="localhost3", state="Absent", available_upto=2147483646
            )
        db.commit()
        yield
...
def test_hulot_wakeup_already_pending(monkeypatch):
    ...
    hulot.nodes_list_running = {
        "localhost2": {"timeout": tools.get_date() + 1000, "command": "WAKEUP"}
    }
    exit_code = hulot.run(False)
    print(hulot.nodes_list_running)
...
def test_hulot_window_forker_check_executors():
    ...
    wf.executors = {
        wf.pool.apply_async(yop, (0,)): ("node1", "HALT", tools.get_date()),
