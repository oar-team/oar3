...
def test_app_media_get_file_not_exit(client):
    ...
    res = client.get(
        url_for("media.get_file", path_filename="yop"),
...
def test_app_media_get_file_tail(client):
    ...
    fake_check_outputs = [b"fake content"]
    res = client.get(
        url_for("media.get_file", path_filename="yop", tail=1),
        headers={"X_REMOTE_IDENT": "bob"},
    )
    assert res.status_code == 200
    print(res.data)
...
def test_app_media_post_file_already_exist(client):
    ...
    temp_path = "~/tmp/yop"
    res = client.post(
        url_for("media.post_file", path_filename=temp_path),
...
def test_app_media_delete_file_not_exit(client):
    ...
    fake_call_retcodes = [1]
    res = client.delete(
        url_for("media.delete", path_filename="yop"), headers={"X_REMOTE_IDENT": "bob"}
    )
    assert res.status_code == 404
...
def test_app_media_chmod_file_not_exit(client):
    ...
    fake_call_retcodes = [1]
    res = client.post(
        url_for("media.chmod", path_filename="yop", mode="755"),
        headers={"X_REMOTE_IDENT": "bob"},
    )
    assert res.status_code == 404
...
def test_app_media_chmod_file_not_alnum(client):
    ...
    fake_call_retcodes = [0]
    res = client.post(
        url_for("media.chmod", path_filename="yop", mode="###"),
        headers={"X_REMOTE_IDENT": "bob"},
    )
