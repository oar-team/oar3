...
def oarwalltime(job_id, new_walltime, force, delay_next_jobs, version, user=None, cli=True):
    ...
    if not new_walltime:        
        ...
        if not walltime_change.walltime:
            cmd_ret.print_('Walltime change status for job {} (job is not running yet):\n  N/A\n'.format(job_id))
        else:
    ...
    (error, _, status, message) = walltime.request(job_id, user, new_walltime, 'YES' if force else 'NO',
