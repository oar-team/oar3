...
from oar.rest_api.proxy_utils import (acquire_lock, release_lock, load_traefik_rules,
                                      save_treafik_rules, del_traefik_rule)

from .utils import CommandReturns


@click.command()
@click.option('--period', default=120, type=int, help="Period between each proxy's rules cleaning (delete rule is associated Job is finished")
def cli(period):
    ...
    while True:
        ...
        try:
            ...
            cmd_ret.error(f'Rest API: proxy_cleaning failed to read proxy rules files {err}')
        finally:
