...
def process_walltime_change_requests(plt, config, logger):
    ...
    for job_wtc in get_jobs_with_walltime_change():
        ...
        fit = job.pending
        if fit > 0:
            apply_time = get_conf(walltime_change_apply_time, job.queue, job.walltime - job.granted, 0)
            increment = get_conf(walltime_increment, job.queue, job.walltime - job.granted, 0)

            logger.debug('[{}] walltime change: pending={}s delay_next_jobs={} force={} apply_time={}s increment={}s'\
                         .format(job_id, fit, job.delay_next_jobs, job.force, apply_time, increment))

                         
            if (not job.force) or (job.force != 'YES'):
                delay = job.start_time + job.walltime + suspended - apply_time - now        
                if apply_time > 0 and delay > 0:
                    logger.debug('[$jobid] walltime change could apply in {}s'.format(job_id, delay))
                    continue # next if activation time is set and activation date in the future
                if increment > 0 and increment < fit:
                    fit = increment
                    
            logger.debug('[{}] walltime change try: {}/{}s'.format(job_id, fit, job.pending))
            ...
            if 'inner' in job_types:
                ...
                if container_job:
                    if (container_job.start_time + container_job.moldable_walltime) < to:
                        to = container_job.start_time + container_job._moldable_walltime #container should never be suspended, makes no sense
                        logger.debug("[{}] walltime change for inner job limited to the container's boundaries: {}s".format(job_id, to - from_))
            ...
            if fit <= 0:
                logger.debug('[{}] walltime cannot be changed for now (pending: {}'\
                             .format(job_id, duration_to_sql_signed(job.pending)))
                continue
        ...
        update_walltime_change_request(job_id, new_pending,
                                       ...
                                       'NO' if (new_pending == 0) else None,
                                       job.granted + fit,
                                       (job.granted_with_force + fit) if (job.force=='YES' and fit>0) else None,
                                       (job.granted_with_delay_next_jobs + fit) if (job.delay_next_jobs=='YES' and fit>0) else None)
