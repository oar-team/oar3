...
schema = db.Table(
    "schema", db.Column("version", db.String(255)), db.Column("name", db.String(255))
)
...
class Accounting(db.Model):
    ...
    window_stop = db.Column(
        db.BigInteger, primary_key=True, autoincrement=False, server_default="0"
    )
    user = db.Column(
        "accounting_user",
        db.String(255),
...
class AssignedResource(db.Model):
    ...
    index = db.Column(
        "assigned_resource_index", db.String(7), index=True, server_default="CURRENT"
    )
...
class File(db.Model):
    ...
    md5sum = db.Column(
        db.String(255), index=True, nullable=True, server_default=db.text("NULL")
    )
...
class JobDependencie(db.Model):
    ...
    index = db.Column(
        "job_dependency_index", db.String(7), index=True, server_default="CURRENT"
    )
...
class JobResourceDescription(db.Model):
    ...
    resource_type = db.Column(
        "res_job_resource_type", db.String(255), primary_key=True, server_default=""
    )
...
class WalltimeChange(db.Model):
    ...
    pending = db.Column(db.Integer, server_default="0")
    force = db.Column(db.String(3), index=True, server_default="NO")
    delay_next_jobs = db.Column(db.String(3), index=True, server_default="NO")
    granted = db.Column(db.Integer, server_default="0")
    granted_with_force = db.Column(db.Integer, server_default="0")
    granted_with_delay_next_jobs = db.Column(db.Integer, server_default="0")

    __table_args__ = (
        db.CheckConstraint(force.in_(["NO", "YES"])),
        db.CheckConstraint(delay_next_jobs.in_(["NO", "YES"])),
    )
