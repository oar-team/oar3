...
from oar.lib.job_handling import (get_job, get_job_types, get_job_suspended_sum_duration,
...
def get_walltime_change_for_job(job_id):
    ...
    try:
        walltime_change = db.query(WalltimeChange).filter(WalltimeChange.job_id == job_id).one()
    except Exception as e:
        logger.debug("get_walltime_change_for_job(" + str(job_id) + ") raises exception: " + str(e))
        return None
    else:
...
def get(job_id):
    ...
    if 'WALLTIME_CHANGE_ENABLED' not in config or config['WALLTIME_CHANGE_ENABLED'] != 'YES':
    ...
    walltime_max_increase = get_conf(config['WALLTIME_MAX_INCREASE'], job.queue_name,
                                     walltime_change.walltime - walltime_change.granted, 0)
    
    walltime_min_for_change = get_conf(config['WALLTIME_MIN_FOR_CHANGE'], job.queue_name, None, 0)
    walltime_users_allowed_to_force = get_conf(config['WALLTIME_ALLOWED_USERS_TO_FORCE'],
                                               job.queue_name, None, '')
    walltime_users_allowed_to_delay_jobs = get_conf(config['WALLTIME_ALLOWED_USERS_TO_DELAY_JOBS'],
                                                    job.queue_name, None, '')
    now = tools.get_date()
    ...
    if walltime_change.walltime != 0:
        ...
        walltime_change.granted_with_force = duration_to_sql_signed(walltime_change.granted_with_force)
        walltime_change.granted_with_delay_next_jobs = duration_to_sql_signed(walltime_change.granted_with_delay_next_jobs)
    else:
...
def request(job_id, user, new_walltime, force, delay_next_jobs):
    ...
    if moldable.walltime < walltime_min_for_change: 
        return (3, 403, 'forbidden',
                'walltime change is not allowed for a job with walltime < {}'.format(walltime_min_for_change_hms))
    ...
    if current_walltime_change:
        if (walltime_max_increase != -1) and (current_walltime_change.granted + new_walltime_seconds > walltime_max_increase) and (user not in ('root','oar')): 
            result = (3, 403, 'forbidden', 'request cannot be updated because the walltime cannot increase by more than {}'.format(walltime_max_increase_hms))
        else:
            ...
            result = (0, 202, 'accepted',
                      'walltime change request updated for job {}, it will be handled shortly'.format(job_id))
    else: # New request
        if (walltime_max_increase != -1) and (new_walltime_seconds > walltime_max_increase) \
           and (user not in ('root','oar')):
            result = (3, 403, 'forbidden',
                      'request cannot be accepted because the walltime cannot increase by more than '.format(walltime_max_increase_hms))
        else:
            add_walltime_change_request(
                ...
                'YES' if (delay_next_jobs and (new_walltime_seconds > 0)) else 'NO')
            result = (0, 202, 'accepted',
                      'walltime change request accepted for job {}, it will be handled shortly'.format(job_id))
...
def add_walltime_change_request(job_id, pending, force, delay_next_jobs):
    ...
    walltime_change  = WalltimeChange(job_id=job_id, pending=pending, force=force,
