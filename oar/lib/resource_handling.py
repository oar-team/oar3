...
def resources_creation(node_name, nb_nodes, nb_core=1, vfactor=1):
    for i in range(nb_nodes * nb_core * vfactor):
        Resource.create(network_address=f"{node_name}{int(i/(nb_core * vfactor)+1)}",
                        cpuset=i % nb_core,
                        state='Alive')
    db.commit()
...
def resources_creation_bulk(node_name, nb_nodes, nb_cpusets=1):
    ...
    resources = [(f"{node_name}{int(i/nb_cpusets)+1}", i % nb_cpusets, 'Alive') for i in range(nb_nodes * nb_cpusets)]

    with db.engine.connect() as to_conn:
        cursor = to_conn.connection.cursor()
        pg_bulk_insert(cursor, db['resources'], resources,
                       ('network_address', 'cpuset', 'state'), binary=True)
