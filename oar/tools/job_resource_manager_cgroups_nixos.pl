# The "job_resource_manager_cgroups_nixos.pl" script is a perl script that oar server
...
if ($ARGV[0] eq "init"){
    ...
    if (defined($Cpuset_path_job)){
        if (open(LOCKFILE,"> $Cpuset->{oar_tmp_directory}/job_manager_lock_file")){
            ...
            if (!(-r $Cgroup_directory_collection_links.'/cpuset/tasks')){
                if (!(-r $OS_cgroups_path.'/cpuset/tasks')){
                    ...
                    if (system('OARDO_BECOME_USER=root oardodo mkdir -p '.$Cgroup_mount_point.' &&
                                OARDO_BECOME_USER=root oardodo mount -t cgroup -o '.$cgroup_list.' none '.$Cgroup_mount_point.' || exit 1
                                OARDO_BECOME_USER=root oardodo rm -f /dev/cpuset
                                OARDO_BECOME_USER=root oardodo ln -s '.$Cgroup_mount_point.' /dev/cpuset &&
                                OARDO_BECOME_USER=root oardodo mkdir -p '.$Cgroup_directory_collection_links.' &&
                                OARDO_BECOME_USER=root oardodo ln -s '.$Cgroup_mount_point.' '.$Cgroup_directory_collection_links.'/cpuset &&
                                OARDO_BECOME_USER=root oardodo ln -s '.$Cgroup_mount_point.' '.$Cgroup_directory_collection_links.'/cpu &&
                                OARDO_BECOME_USER=root oardodo ln -s '.$Cgroup_mount_point.' '.$Cgroup_directory_collection_links.'/cpuacct &&
                                OARDO_BECOME_USER=root oardodo ln -s '.$Cgroup_mount_point.' '.$Cgroup_directory_collection_links.'/devices &&
                                OARDO_BECOME_USER=root oardodo ln -s '.$Cgroup_mount_point.' '.$Cgroup_directory_collection_links.'/freezer &&
                                OARDO_BECOME_USER=root oardodo ln -s '.$Cgroup_mount_point.' '.$Cgroup_directory_collection_links.'/blkio &&
                                [ "'.$ENABLE_MEMCG.'" =  "YES" ] && OARDO_BECOME_USER=root oardodo ln -s '.$Cgroup_mount_point.' '.$Cgroup_directory_collection_links.'/memory || true
                               ')){
                        exit_myself(4,"Failed to mount cgroup pseudo filesystem");
                    }
                }else{
                    # Cgroups already mounted by the OS
                    if (system('OARDO_BECOME_USER=root oardodo rm -f /dev/cpuset
                                OARDO_BECOME_USER=root oardodo ln -s '.$OS_cgroups_path.'/cpuset /dev/cpuset &&
                                OARDO_BECOME_USER=root oardodo mkdir -p '.$Cgroup_directory_collection_links.' &&
                                OARDO_BECOME_USER=root oardodo ln -s '.$OS_cgroups_path.'/cpuset '.$Cgroup_directory_collection_links.'/cpuset &&
                                OARDO_BECOME_USER=root oardodo ln -s '.$OS_cgroups_path.'/cpu '.$Cgroup_directory_collection_links.'/cpu &&
                                OARDO_BECOME_USER=root oardodo ln -s '.$OS_cgroups_path.'/cpuacct '.$Cgroup_directory_collection_links.'/cpuacct &&
                                OARDO_BECOME_USER=root oardodo ln -s '.$OS_cgroups_path.'/devices '.$Cgroup_directory_collection_links.'/devices &&
                                OARDO_BECOME_USER=root oardodo ln -s '.$OS_cgroups_path.'/freezer '.$Cgroup_directory_collection_links.'/freezer &&
                                OARDO_BECOME_USER=root oardodo ln -s '.$OS_cgroups_path.'/blkio '.$Cgroup_directory_collection_links.'/blkio &&
                                [ "'.$ENABLE_MEMCG.'" =  "YES" ] && OARDO_BECOME_USER=root oardodo ln -s '.$OS_cgroups_path.'/memory '.$Cgroup_directory_collection_links.'/memory || true
                               ')){
            ...
            if (!(-d $Cgroup_directory_collection_links.'/cpuset/'.$Cpuset->{cpuset_path})){
                ...
                if (system( 'for d in '.$Cgroup_directory_collection_links.'/*; do
                               OARDO_BECOME_USER=root oardodo mkdir -p $d/'.$Cpuset->{cpuset_path}.' || exit 1
                               OARDO_BECOME_USER=root oardodo chown -R oar $d/'.$Cpuset->{cpuset_path}.' || exit 2
                               /run/current-system/sw/bin/echo 0 | cat > $d/'.$Cpuset->{cpuset_path}.'/notify_on_release || exit 3
                             done
                             /run/current-system/sw/bin/echo 0 | cat > '.$Cgroup_directory_collection_links.'/cpuset/'.$Cpuset->{cpuset_path}.'/cpuset.cpu_exclusive &&
                             ...
                             /run/current-system/sw/bin/echo 1000 | cat > '.$Cgroup_directory_collection_links.'/blkio/'.$Cpuset->{cpuset_path}.'/blkio.weight
                            ')){
        ...
        # Create job cgroup
        if (system( 'for d in '.$Cgroup_directory_collection_links.'/*; do
                       OARDO_BECOME_USER=root oardodo mkdir -p $d/'.$Cpuset_path_job.' || exit 1
                       OARDO_BECOME_USER=root oardodo chown -R oar $d/'.$Cpuset_path_job.' || exit 2
                       /run/current-system/sw/bin/echo 0 | cat > $d/'.$Cpuset_path_job.'/notify_on_release || exit 3
                     done
                     /run/current-system/sw/bin/echo 0 | cat > '.$Cgroup_directory_collection_links.'/cpuset/'.$Cpuset_path_job.'/cpuset.cpu_exclusive &&
                     MEM=
                     ...
                     /run/current-system/sw/bin/echo '.join(",",@Cpuset_cpus).' | cat > '.$Cgroup_directory_collection_links.'/cpuset/'.$Cpuset_path_job.'/cpuset.cpus
                    ')){
        ...
        $IO_ratio = 1000;
        if (system( '/run/current-system/sw/bin/echo '.$IO_ratio.' | cat > '.$Cgroup_directory_collection_links.'/blkio/'.$Cpuset_path_job.'/blkio.weight')){
        ...
        if ($ENABLE_DEVICESCG eq "YES"){
            ...
            if ($#devices_deny > -1){
                ...
                foreach my $dev (@devices_deny){
                    if (system("OARDO_BECOME_USER=root oardodo /run/current-system/sw/bin/echo 'c 195:$dev rwm' > $devices_cgroup")) {
        ...
        if ($ENABLE_MEMCG eq "YES"){
            ...
            if (system( '/run/current-system/sw/echo '.$mem_kb.' | cat > '.$Cgroup_directory_collection_links.'/memory/'.$Cpuset_path_job.'/memory.limit_in_bytes')){
    ...
    if (defined($Cpuset_path_job)){
        ...
        if (open(LOCK,">", $Cpuset_lock_file.$Cpuset->{user})){
            ...
            if (system('if [ -w '.$Cgroup_directory_collection_links.'/cpuset/'.$Cpuset_path_job.'/memory.force_empty ]; then
                        ...
                        OARDO_BECOME_USER=root oardodo rmdir '.$Cgroup_directory_collection_links.'/cpuset/'.$Cpuset_path_job.' &&
                        for d in '.$Cgroup_directory_collection_links.'/*/'.$Cpuset_path_job.'; do
                          [ -w $d/memory.force_empty ] && echo 0 > $d/memory.force_empty
                          if [ -d $d ]; then
                            OARDO_BECOME_USER=root oardodo rmdir $d > /dev/null 2>&1 || exit 1
                          fi
                        done
                       ')){
            ...
            if ($#cpusets < 0) {
                ...
                system('for d in '."@TMP_DIRECTORIES_TO_CLEAR".'; do
                            OARDO_BECOME_USER=root oardodo find $d -user '.$Cpuset->{user}.' -delete
