...
if ($ARGV[0] eq "init"){
    ...
    if (defined($Cpuset_path_job)){
        if (open(LOCKFILE,"> $Cpuset->{oar_tmp_directory}/job_manager_lock_file")){
            ...
            if (!(-r $Cgroup_directory_collection_links.'/cpuset/tasks')){
                ...
                if (!(-r $OS_cgroups_path.'/cpuset/tasks')){
                    system('set -e
                            OARDO_BECOME_USER=root oardodo mkdir -p '.$Cgroup_mount_point.'
                            OARDO_BECOME_USER=root oardodo mount -t cgroup -o '.join(',',@cgroup_list).' none '.$Cgroup_mount_point.'
                            ...
                            for cg in '.join(' ',@cgroup_list).'; do
                                OARDO_BECOME_USER=root oardodo ln -s '.$Cgroup_mount_point.' '.$Cgroup_directory_collection_links.'/$cg
                            done')
                    ...
                    system('set -e
                            OARDO_BECOME_USER=root oardodo rm -f /dev/cpuset
                            OARDO_BECOME_USER=root oardodo ln -s '.$OS_cgroups_path.'/cpuset'.$Oardocker_node_cg_path.' /dev/cpuset
                            OARDO_BECOME_USER=root oardodo mkdir -p '.$Cgroup_directory_collection_links.'
                            for cg in '.join(' ',@cgroup_list).'; do
                                OARDO_BECOME_USER=root oardodo ln -s '.$OS_cgroups_path.'/$cg'.$Oardocker_node_cg_path.' '.$Cgroup_directory_collection_links.'/$cg
                            done')
            ...
            if (!(-d $Cgroup_directory_collection_links.'/cpuset/'.$Cpuset->{cpuset_path})){
                ...
                system('set -e
                        for d in '.$Cgroup_directory_collection_links.'/*; do
                            OARDO_BECOME_USER=root oardodo mkdir -p $d/'.$Cpuset->{cpuset_path}.'
                            OARDO_BECOME_USER=root oardodo chown -R oar $d/'.$Cpuset->{cpuset_path}.'
                            /run/current-system/sw/bin/echo 0 | /run/current-system/sw/bin/cat > $d/'.$Cpuset->{cpuset_path}.'/notify_on_release
                        done
                        /run/current-system/sw/bin/echo 0 | /run/current-system/sw/bin/cat > '.$Cgroup_directory_collection_links.'/cpuset/'.$Cpuset->{cpuset_path}.'/cpuset.cpu_exclusive
                        /run/current-system/sw/bin/cat '.$Cgroup_directory_collection_links.'/cpuset/cpuset.mems > '.$Cgroup_directory_collection_links.'/cpuset/'.$Cpuset->{cpuset_path}.'/cpuset.mems
                        /run/current-system/sw/bin/cat '.$Cgroup_directory_collection_links.'/cpuset/cpuset.cpus > '.$Cgroup_directory_collection_links.'/cpuset/'.$Cpuset->{cpuset_path}.'/cpuset.cpus')
                and exit_myself(4,"Failed to create cgroup $Cpuset->{cpuset_path}");
                if ($Enable_blkio_cg eq "YES") {
                    system('/run/current-system/sw/bin/echo 1000 | /run/current-system/sw/bin/cat > '.$Cgroup_directory_collection_links.'/blkio/'.$Cpuset->{cpuset_path}.'/blkio.weight')
                    and exit_myself(4,"Failed to create cgroup $Cpuset->{cpuset_path}");
                }
            }
        ...
        if (open(LOCK,">", $Cpuset_lock_file.$Cpuset->{user})){
            ...
            my $job_cpuset_cpus_cmd = '/run/current-system/sw/bin/echo '.join(",",@Cpuset_cpus).' | /run/current-system/sw/bin/cat > '.$Cgroup_directory_collection_links.'/cpuset/'.$Cpuset_path_job.'/cpuset.cpus';
            if (exists($Cpuset->{'compute_thread_siblings'}) and lc($Cpuset->{'compute_thread_siblings'}) eq "yes") {
                ...
                $job_cpuset_cpus_cmd = 'for i in '.join(" ", map {s/,/ /gr} @Cpuset_cpus).'; do /run/current-system/sw/bin/cat /sys/devices/system/cpu/cpu$i/topology/thread_siblings_list; done | paste -sd, - > '.$Cgroup_directory_collection_links.'/cpuset/'.$Cpuset_path_job.'/cpuset.cpus';
            }
            system('set -e
                    for d in '.$Cgroup_directory_collection_links.'/*; do
                        OARDO_BECOME_USER=root oardodo mkdir -p $d/'.$Cpuset_path_job.'
                        OARDO_BECOME_USER=root oardodo chown -R oar $d/'.$Cpuset_path_job.'
                        /run/current-system/sw/bin/echo 0 | /run/current-system/sw/bin/cat > $d/'.$Cpuset_path_job.'/notify_on_release
                    done
                    /run/current-system/sw/bin/echo 0 | /run/current-system/sw/bin/cat > '.$Cgroup_directory_collection_links.'/cpuset/'.$Cpuset_path_job.'/cpuset.cpu_exclusive
                    '.$job_cpuset_cpus_cmd)
            and exit_myself(5,"Failed to create and feed the cpuset cpus $Cpuset_path_job");
            if ($Cpuset_cg_mem_nodes eq "all"){
                system('/run/current-system/sw/bin/cat '.$Cgroup_directory_collection_links.'/cpuset/cpuset.mems > '.$Cgroup_directory_collection_links.'/cpuset/'.$Cpuset_path_job.'/cpuset.mems')
        ...
        if ($Enable_net_cls_cg eq "YES") {
            system( '/run/current-system/sw/bin/echo '.$Cpuset->{job_id}.' | /run/current-system/sw/bin/cat > '.$Cgroup_directory_collection_links.'/net_cls/'.$Cpuset_path_job.'/net_cls.classid')
        ...
        if ($Enable_blkio_cg eq "YES") {
            ...
            $IO_ratio = 1000;
            system( '/run/current-system/sw/bin/echo '.$IO_ratio.' | /run/current-system/sw/bin/cat > '.$Cgroup_directory_collection_links.'/blkio/'.$Cpuset_path_job.'/blkio.weight')
        ...
        if ($Enable_devices_cg eq "YES"){
            ...
            if ($#devices_deny > -1){
                ...
                foreach my $dev (@devices_deny){
                    system("OARDO_BECOME_USER=root oardodo /run/current-system/sw/bin/echo 'c 195:$dev rwm' > $devices_cgroup")
        ...
        if ($Enable_mem_cg eq "YES"){
            ...
            system('/run/current-system/sw/bin/echo '.$mem_kb.' | /run/current-system/sw/bin/cat > '.$Cgroup_directory_collection_links.'/memory/'.$Cpuset_path_job.'/memory.limit_in_bytes')
